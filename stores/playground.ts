...
export const usePlaygroundStore = defineStore('playground', () => {
  ...
  if (import.meta.client) {
    async function mount() {
      ...
      const wc = await import('@webcontainer/api').then(({ WebContainer }) =>
        WebContainer.boot(),
      )
  ...
  async function startServer(reinstall = false) {
    ...
    if (reinstall) hasInstalled = false

    if (!hasInstalled) await launchInstallProcess(wc, signal)

    if (hasInstalled) await launchNuxtProcess(wc, signal)
  ...
  async function launchInteractiveProcess(
    wc: WebContainer,
    signal: AbortSignal,
  ) {
    if (signal.aborted) return
    ...
    const crawlFiles = async (dir: string, zip: Zip) => {
      ...
      await Promise.all(
        files.map(async (file) => {
          if (isFileIgnored(file.name)) return
  ...
  async function _mountFiles(overrides: Record<string, string>) {
    await Promise.all(
      Object.entries(overrides).map(async ([filepath, content]) => {
