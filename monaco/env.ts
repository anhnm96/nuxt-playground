...
export type PlaygroundMonacoContext = Pick<
  PlaygroundStore,
  'webcontainer' | 'files'
>
...
export class WorkerHost {
  ...
  async fsReadFile(uriString: string, encoding = 'utf-8') {
    ...
    try {
      ...
      const content = await this.ctx.webcontainer!.fs.readFile(
        filepath,
        encoding as 'utf-8',
      )
      if (content != null) getOrCreateModel(uri, undefined, content)
      return content
    } catch (err) {
      console.error(err)
      return undefined
  ...
  async fsStat(uriString: string) {
    ...
    try {
      ...
      const files = await this.ctx.webcontainer!.fs.readdir(dir, {
        withFileTypes: true,
      })
  ...
  async fsReadDirectory(uriString: string) {
    ...
    try {
      const filepath = withoutLeadingSlash(uri.fsPath)
      const result = await this.ctx.webcontainer!.fs.readdir(filepath, {
        withFileTypes: true,
      })
      return result.map((item) => [item.name, item.isDirectory() ? 2 : 1]) as [
        string,
        1 | 2,
      ][]
    } catch (err) {
...
export async function reloadLanguageTools(ctx: PlaygroundMonacoContext) {
  ...
  function resolvePath(path: string) {
    if (path.startsWith('./')) return `./.nuxt/${path.slice(2)}`
  ...
  const getSyncUris = () => [
    ...Array.from(ctx.files.values()).map((file) =>
